...
const { loadNavData, normalizeAirportRows, DEFAULT_NAVDATA_BASE_URL } = require('./csv/githubNavData');
...
async function main() {
  ...
  const navData = await loadNavData({ dataDir, baseUrl: config.navDataBaseUrl || DEFAULT_NAVDATA_BASE_URL, preferRemote: config.preferRemoteNavData });
  ...
  const groundSequencer = new GroundSequencer({ airports });
  ...
  wss.on('connection', (ws) => { clients.add(ws); ws.send(JSON.stringify({ type:'hello', service:'skyecho-backend-v1.1', traffic: traffic.snapshot(), data: { source: navData.source, counts: navData.counts, errors: navData.errors.slice(0,8) } })); ws.on('close', () => clients.delete(ws)); });
  ...
  app.get('/', (req,res)=>res.json({ ok:true, service:'SkyEcho Backend v1.1 AI Traffic Orchestrator', ws:'/ws', health:'/health', dataStatus:'/data/status' }));
  app.get('/health', (req,res)=>res.json({ ok:true, version:'1.1.0', airports:airports.size, airlines:airlineRegistry.size, navSource:navData.source, navFilesLoaded:Object.values(navData.counts).filter(n=>n>0).length, running:traffic.running, clients:clients.size }));
  app.get('/data/status', (req,res)=>res.json({ ok:true, source:navData.source, baseUrl:config.navDataBaseUrl, counts:navData.counts, errors:navData.errors, normalizedAirportRows:airportRows.length, airportDbSize:airports.size }));
  ...
  server.listen(config.port, () => console.log(`SkyEcho Backend v1.1 listening on ${config.port}; nav files loaded=${Object.values(navData.counts).filter(n=>n>0).length}; airports=${airports.size}`));
}
