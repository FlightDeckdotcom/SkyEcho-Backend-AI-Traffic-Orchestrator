...
const VERSION = "6.9.70-safe-rollback";
...
const runtime = {
  fs2efb: { lastPoll: null, lastIngest: null, lastError: null, lastData: null },
...
function safeJson(text) { try { return typeof text === "string" ? JSON.parse(text) : (text || {}); } catch { return { raw: text }; } }
function num(v) { const n = Number(v); return Number.isFinite(n) ? n : null; }

function normalizeFs2EfbTelemetry(raw = {}) {
  const src = raw?.aircraft || raw?.telemetry || raw?.data || raw?.state || raw || {};
  const pos = src.position || src.pos || raw.position || {};
  return {
    source: "fs2efb",
    timestamp: new Date().toISOString(),
    callsign: src.callsign || src.callSign || src.flight || src.atc_id || raw.callsign || "",
    latitude: num(src.lat ?? src.latitude ?? pos.lat ?? pos.latitude),
    longitude: num(src.lon ?? src.lng ?? src.longitude ?? pos.lon ?? pos.lng ?? pos.longitude),
    altitude: num(src.altitude ?? src.alt ?? src.baro_altitude ?? pos.altitude),
    heading: num(src.heading ?? src.hdg ?? src.track),
    groundSpeed: num(src.groundSpeed ?? src.groundspeed ?? src.gs ?? src.speed),
    indicatedAirspeed: num(src.ias ?? src.indicatedAirspeed),
    verticalSpeed: num(src.verticalSpeed ?? src.vs ?? src.vertical_speed),
    onGround: Boolean(src.onGround ?? src.simOnGround ?? src.ground),
    com1: src.com1 ?? src.com1Active ?? src.radio ?? src.frequency ?? null,
...
async function handleTelemetry(telemetry) {
  ...
  const engine = ingestTelemetry(telemetry);
  ...
  const si = await sendToSayIntentionsIfEnabled(
    ...
    { enabled: SAYINTENTIONS_ADAPTER_ENABLED, url: SAYINTENTIONS_SIMAPI_URL, apiKey: SAYINTENTIONS_API_KEY }
  );
  runtime.lastSayIntentionsForward = si;
  return { engine, volanta, sayIntentions: si };
}
...
app.get("/", (req, res) => res.json({ ok: true, service: "SkyEchoCabin Safe Rollback", version: VERSION, app: "/app/" }));
app.get(["/app", "/app/"], (req, res) => res.sendFile(path.join(__dirname, "frontend", "index.html")));
app.get("/health", (req, res) => res.json({
  ok: true,
  version: VERSION,
  rollback: true,
  note: "v7 nav engine disabled. Using original skyEchoAtcEngine flow.",
  ...
  trafficStopped: runtime.trafficStopped,
  engine: getEngineHealth()
}));

app.post("/api/session/start", (req, res) => {
  const f = req.body || {};
  const session = createAtcSession({
    callsign: f.callsign || f.flightNumber || "AAL318",
    aircraft: f.aircraft || f.aircraftType || "B738",
    origin: f.origin || f.departure || "",
    destination: f.destination || f.dest || "",
    route: f.route || "",
    cruiseAltitude: f.cruiseAltitude || f.cruise || "",
    simbriefId: f.simbriefId || SIMBRIEF_USER_ID || "",
    runway: f.runway || ""
  });
  startVolantaSession(f);
  res.json({ ok: true, session });
});

app.get("/api/session", (req, res) => res.json({ ok: true, session: getAtcSession() }));

app.post("/api/pilot/transmit", (req, res) => {
  const text = req.body?.text || req.body?.transcript || "";
  res.json({ ok: true, ...handlePilotTransmission(text, { telemetry: runtime.fs2efb.lastData }) });
});

// Keep endpoint so newer frontend does not 404, but route it to the original engine.
app.post("/api/atc/respond", (req, res) => {
  const text = req.body?.text || req.body?.transcript || "";
  const result = handlePilotTransmission(text, { telemetry: runtime.fs2efb.lastData });
  const atcText = result.atcResponseText || result.atc || result.response || result.text || "";
  res.json({ ok: true, ...result, atcResponseText: atcText, atc: atcText, response: atcText, engine: getAtcSession() });
});

app.post("/api/route/parse", (req, res) => res.json({ ok: true, parsed: parseSimBriefLikeRoute(req.body?.route || req.body?.text || "") }));
app.get("/api/fs2efb", async (req, res) => { const result = await pollFs2Efb(); res.status(result.ok ? 200 : 502).json(result); });
...
app.get("/volanta/snapshot", (req, res) => res.json({ ok: true, snapshot: buildVolantaSnapshot(runtime.fs2efb.lastData) }));

app.get("/sayintentions/adapter/health", (req, res) => res.json({
  ok: true,
...
app.get("/traffic-v2/health", (req, res) => res.json({
  ...
  stopped: runtime.trafficStopped,
  running: !runtime.trafficStopped,
  fs2efbConfigured: Boolean(FS2EFB_URL),
...
app.get("/traffic-v2/state", async (req, res) => {
  ...
  res.json({
    ...
    running: !runtime.trafficStopped,
    stopped: runtime.trafficStopped,
    aircraftCount: runtime.trafficStopped ? 0 : (runtime.fs2efb.lastData ? 1 : 0),
    aircraft: runtime.trafficStopped ? [] : (runtime.fs2efb.lastData ? [runtime.fs2efb.lastData] : []),
    queue: [],
    audioQueue: [],
    engine: getAtcSession(),
    lastError: runtime.fs2efb.lastError
...
app.post("/traffic-v2/start", async (req, res) => {
  runtime.trafficStopped = false;
  if (!getAtcSession().active || req.body?.route) {
    createAtcSession({
...
app.post("/traffic-v2/stop", (req, res) => {
  runtime.trafficStopped = true;
  res.json({ ok: true, running: false, stopped: true, cleared: true, queue: [], audioQueue: [] });
});

app.use((req, res) => res.status(404).json({ ok: false, error: `Cannot ${req.method} ${req.path}` }));
app.listen(PORT, () => console.log(`SkyEchoCabin Safe Rollback ${VERSION} on ${PORT}`));
